...
def evaluate_batch(sess,accuracy,cifar_dataset,image_pl,label_pl,steps_per_epoch,summary_writer,merged,current_epoch,keep_prob_fc1,keep_prob_fc2,training):
    # Inputs:
        ...
        # keep_prob_fc1 : Dropout parameter for the first fully connected layer
        # keep_prob_fc2 : Dropout parameter for the second fully connected layer
    ...
    for x in range(steps_per_epoch):
        ...
        if training:
            summary,current_count = sess.run([merged,accuracy],feed_dict = {image_pl: images_batch,label_pl:label_batch,keep_prob_fc1 : 0.5,keep_prob_fc2 : 0.5 })
        else:
            summary,current_count = sess.run([merged,accuracy],feed_dict = {image_pl: images_batch,label_pl:label_batch,keep_prob_fc1 : 1.0,keep_prob_fc2 : 1.0})
...
def run_training():
    ...
    with tf.Graph().as_default():
        ...
        # Construct the graph
        out,regularizer,keep_prob_fc1,keep_prob_fc2 = cifar.inference(image,training = True)
        ...
        for i in range(num_epochs):
            ...
            for j in range(steps_per_epoch_train):
                ...
                summary,_ = sess.run([merged,train_step],feed_dict = {image:images_batch,label:label_batch,keep_prob_fc1 : 0.5,keep_prob_fc2 : 0.5})
            if i%10 == 0 :
                # Record current training loss
                train_writer.add_summary(summary,i)
            # Check accuracy every epoch
            train_accuracy = evaluate_batch(sess,accuracy,cifar_dataset,image,label,steps_per_epoch_train,train_writer,merged,i,keep_prob_fc1,keep_prob_fc2,training = True) #evaluate model with training data
            print('Epoch '+str(i)+' training accuracy: '+str(train_accuracy))
            test_accuracy = evaluate_batch(sess,accuracy,cifar_dataset,image,label,steps_per_epoch_test,test_writer,merged,i,keep_prob_fc1,keep_prob_fc2,training = False) #evaluate model with test data
